...
Test(http, should_parse_valid_request_line) {
    ...
    cr_assert(parse_http_request_line("GET /index.html HTTP/1.1", &request) == PARSE_OK, "Valid request line should be parsed");
...
Test(http, should_not_parse_malformed_request_line_missing_parts) {
    http_request_t request = {0};
    cr_assert(parse_http_request_line("GET /index.html", &request) == PARSE_MALFORMED_REQUEST_LINE, "Request line missing protocol should not be parsed");
    cr_assert(parse_http_request_line("GET", &request) == PARSE_MALFORMED_REQUEST_LINE, "Request line missing path and protocol should not be parsed");
    cr_assert(parse_http_request_line("", &request) == PARSE_MALFORMED_REQUEST_LINE, "Empty request line should not be parsed");
}

Test(http, should_not_parse_request_line_with_invalid_method) {
    http_request_t request = {0};
    cr_assert(parse_http_request_line("POST /index.html HTTP/1.1", &request) == PARSE_INVALID_METHOD, "Request line with invalid method should not be parsed");
    cr_assert(parse_http_request_line("DELETE /index.html HTTP/1.1", &request) == PARSE_INVALID_METHOD, "Request line with invalid method should not be parsed");
}

Test(http, should_not_parse_request_line_with_invalid_path) {
    http_request_t request = {0};
    cr_assert(parse_http_request_line("GET index.html HTTP/1.1", &request) == PARSE_INVALID_PATH, "Request line with path missing leading slash should not be parsed");
    cr_assert(parse_http_request_line("GET \"\" HTTP/1.1", &request) == PARSE_INVALID_PATH, "Request line with empty path should not be parsed");
}

Test(http, should_not_parse_request_line_with_invalid_protocol) {
    http_request_t request = {0};
    cr_assert(parse_http_request_line("GET /index.html HTTP/1.0", &request) == PARSE_INVALID_PROTOCOL, "Request line with invalid protocol should not be parsed");
    cr_assert(parse_http_request_line("GET /index.html HTTP/2.0", &request) == PARSE_INVALID_PROTOCOL, "Request line with invalid protocol should not be parsed");
    cr_assert(parse_http_request_line("GET /index.html HTTPS/1.1", &request) == PARSE_INVALID_PROTOCOL, "Request line with invalid protocol should not be parsed");
}
...
Test(http, should_not_parse_http_request_without_crlf) {
    http_request_t request = {0};
    const char *http_data = "GET /test.html HTTP/1.1";
    cr_assert(parse_http_request(http_data, &request) == PARSE_UNTERMINATED_REQUEST_LINE, "HTTP request without CRLF should not be parsed");
}
...
Test(http, should_not_parse_http_request_with_too_long_request_line) {
    ...
    cr_assert(parse_http_request(long_line, &request) == PARSE_MALFORMED_REQUEST_LINE, "HTTP request with too long request line should not be parsed");
}

Test(http, should_parse_request_line_with_minimal_valid_path) {
    http_request_t request = {0};
    cr_assert(parse_http_request_line("GET / HTTP/1.1", &request) == PARSE_OK, "Request line with root path should be parsed");
    cr_assert_str_eq(request.path, "/", "Path should be /");
}

Test(http, should_parse_request_line_with_complex_valid_path) {
    http_request_t request = {0};
    cr_assert(parse_http_request_line("GET /path/to/resource.html HTTP/1.1", &request) == PARSE_OK, "Request line with complex path should be parsed");
